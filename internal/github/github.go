...
func (c *Client) CreatePull(ctx context.Context, head, base, title, body string, draft bool) error {
	ghClient := ghClient(ctx, c.token)
	_, _, err := ghClient.PullRequests.Create(ctx,
...
func (c *Client) UpdatePull(ctx context.Context, pull int, base, title, body string, draft bool) error {
	ghClient := ghClient(ctx, c.token)
	_, _, err := ghClient.PullRequests.Edit(ctx,
